...
namespace {
    ...
    void MyTest() {
        ...
        sheet->SetCell("A2"_pos, "=2+2");
        sheet->SetCell("A3"_pos, "=A1+A2");

        {
            CellInterface* cell = sheet->GetCell("A1"_pos);
            ASSERT_EQUAL(cell->GetText(), "1");
            ASSERT_EQUAL(std::get<std::string>(cell->GetValue()), "1");
        }

        {
            CellInterface* cell = sheet->GetCell("A2"_pos);
            ASSERT_EQUAL(cell->GetText(), "=2+2");
            ASSERT_EQUAL(std::get<double>(cell->GetValue()), 4);
        }

        {
            CellInterface* cell = sheet->GetCell("A3"_pos);
            ASSERT_EQUAL(cell->GetText(), "=A1+A2");
